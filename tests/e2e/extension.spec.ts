...
test("shows inline unit-price sort when the retailer has no visible sort control", async ({ page }) => {
  ...
  await expect(page.locator("[data-ata-sort-control]")).toBeVisible();
  await expect(productTitles.first()).toContainText("Premium Clay Cat Litter");

  await page.getByRole("button", { name: "Sort by unit price" }).click();
  await expect(productTitles.first()).toContainText("Special Kitty Non-Clumping");
...
test("adds unit-price sort to Walmart-style label-only sort popovers", async ({ page }) => {
  ...
  await expect(unitSortOption.locator("label")).toHaveText("Unit Price");
